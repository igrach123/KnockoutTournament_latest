...
const KnockoutTournamentGenerator = () => {
  ...
  useEffect(() => {
    ...
    if (savedMatches !== null) setMatches(JSON.parse(savedMatches));
    if (savedRound !== null) setRound(parseInt(savedRound, 10));
    if (savedCompletedRounds !== null)
      setCompletedRounds(parseInt(savedCompletedRounds, 10));
    if (savedHistory !== null) setHistory(JSON.parse(savedHistory));
    if (savedTitle !== null) setTournamentTitle(savedTitle);
  }, []);
